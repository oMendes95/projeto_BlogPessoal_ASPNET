...
namespace BlogAPI.Src.Controladores
{
    ...
    public class PostagemControlador : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> NovaPostagemAsync([FromBody] Postagem idPostagem)
        {
            try
            {
                await _repositorio.NovaPostagemAsync(idPostagem);
                return Created($"api/Postagens", idPostagem);
            }
            catch (Exception ex)
            {
